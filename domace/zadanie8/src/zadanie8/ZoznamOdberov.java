...
public class ZoznamOdberov {
	...
	public boolean prekonalNakazu(String menoZamestnanca) {
		// Evalvator mi hlasi chybu ale osobne si myslim ze to mam dobre kedze najprv mal test pozitivny a potom negativny
		// Myslim si ze problem je v tom ze to porovnam i napriek tomu ze to je v rovnaky den.
	...
	public List podozrivoTestovani() {
		...
		for (Object object : odbery) {
			if (object instanceof Odber) {
				...
				//2 foreach cykly pre porovnanie kazdeho odberu s kazdym
	...
	public int najvytazenejsiePracovisko() {
		//Map Integer(miesto), Integer(pocetTestov)
		...
		for (Object object : odbery) {
			if (object instanceof Odber) {
				...
				//ak sa odberne miesto nenachadza pridaj ho do mapy a nastav hodnotu 1
				if (!odberneMiesta.containsKey(o.getOdberneMiesto())) {
					odberneMiesta.put(o.getOdberneMiesto(), 1);
				} else {
					//ak sa odberne miesto nachadza pridaj mu hodnotu + 1
		...
		for (Integer kluc : odberneMiesta.keySet()) {
			...
			//ak najdem vacsi pocet testov na pracovisku
			if (odberneMiesta.get(kluc) > max) {
				
				//ulozim si pocet testov do max
				max = odberneMiesta.get(kluc);
				// a pracovisko = kluc pretoze chcem vratit pracovisko
				pracovisko = kluc;
			}
		}
